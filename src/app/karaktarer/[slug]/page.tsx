...
const CAT = 'karaktarer' as const
const CAT_LABEL = 'Karaktärer'
...
export default async function KaraktarerArticle({ params }: Props) {
  ...
  return (
    <>
      <Header />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ '@context': 'https://schema.org', '@type': 'Article', headline: article.title, description: article.description, inLanguage: 'sv-SE', about: { '@type': 'Person', name: article.title } }) }} />
      <main style={{ paddingTop: '88px' }}>
        <section style={{ padding: '3.5rem 1rem 2.5rem', position: 'relative', overflow: 'hidden', background: `linear-gradient(180deg, #100C15 0%, #07040A 100%)`, borderBottom: `1px solid ${CAT_COLOR}15` }}>
          ...
          <div style={{ maxWidth: '740px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
            ...
            <span className="cat-badge neon-text-purple" style={{ color: CAT_COLOR, display: 'inline-block', marginBottom: '0.9rem', background: `${CAT_COLOR}0A`, padding: '0.2rem 0.6rem', borderRadius: '2px' }}>{CAT_LABEL}</span>
