...
export default function ArticleCard({ article, featured = false }: Props) {
  ...
  if (featured) {
    return (
      <Link href={`/${article.category}/${article.slug}`} className="article-card block" style={{
        ...
        border: '1px solid #1A1325', borderLeft: `3px solid ${c}`,
        borderRadius: '2px', padding: 'clamp(1.75rem, 3vw, 2.75rem)',
        textDecoration: 'none', position: 'relative', overflow: 'hidden',
      }}>
        {/* Stamp */}
        <span className="cat-badge" style={{
          color: c, display: 'inline-block', marginBottom: '1rem',
          textShadow: `0 0 10px ${c}40`,
          background: `${c}0C`, padding: '0.2rem 0.7rem', borderRadius: '2px',
  ...
  return (
    <Link href={`/${article.category}/${article.slug}`} className="article-card vice-card block" style={{
      borderLeft: `3px solid ${c}`, padding: '1.4rem 1.5rem 1.4rem 1.65rem',
      textDecoration: 'none', position: 'relative', overflow: 'hidden',
    }}>
      {/* Tilted stamp badge */}
      <span className="cat-badge" style={{
        color: c, display: 'inline-block', marginBottom: '0.65rem',
        textShadow: `0 0 8px ${c}30`,
        background: `${c}0A`, padding: '0.15rem 0.5rem', borderRadius: '2px',
